...
@interface QAdPicView ()<UIScrollViewDelegate>
{
...
@implementation QAdPicView

+ (instancetype)picViewWithLoadImageBlock: (LoadImageBlock)loadBlock {

    QAdPicView *adView = [[QAdPicView alloc] init];
...
-(void)setPicModels:(NSArray<id <QAdPicProtocol>> *)picModels
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<QAdPicProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<QAdPicProtocol> adM = self.picModels[tag];
